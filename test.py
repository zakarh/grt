...
def graph(tmp_path: Path):
    ...
    test_dir: Path = tmp_path / "test.graph"
    grt: GRT = GRT(directory=str(test_dir))
    yield grt
    # Cleanup is automatic with tmp_path; if needed, you could add:
    # shutil.rmtree(test_dir, ign   ore_errors=True)


# Test Node Creation
def test_create_node(graph: GRT):
...
# Test Node Update
def test_update_node(graph: GRT):
    graph.nodes.create("node1", {"property": "value"})
...
# Test Node Deletion
def test_delete_node(graph: GRT):
    graph.nodes.create("node1", {"property": "value"})
...
# Test Edge Creation
def test_create_edge(graph: GRT):
...
# Test Edge Update
def test_update_edge(graph: GRT):
    graph.nodes.create("node1")
    graph.nodes.create("node2")
    graph.edges.create("node1", "node2", {"weight": 5})
...
# Test Edge Deletion
def test_delete_edge(graph: GRT):
    graph.nodes.create("node1")
    graph.nodes.create("node2")
    graph.edges.create("node1", "node2", {"weight": 5})
...
def test_get_all_nodes(graph: GRT):
    graph.nodes.create("node1", {"property": "value"})
...
def test_get_all_edges(graph: GRT):
    graph.nodes.create("node1")
    graph.nodes.create("node2")
    graph.edges.create("node1", "node2", {"weight": 5})
...
def test_incoming_outgoing_edges(graph: GRT):
    graph.nodes.create("node1")
    graph.nodes.create("node2")
...
def test_delete_node_deletes_edges(graph: GRT):
    graph.nodes.create("node1")
    graph.nodes.create("node2")
    graph.edges.create("node1", "node2", {"weight": 5})
